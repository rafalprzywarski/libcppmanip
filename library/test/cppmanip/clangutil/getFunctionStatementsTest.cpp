...
#include <cppmanip/clangutil/getStmtSpecificOffsetRanges.hpp>
...
struct getFunctionStatementsTest : testing::Test
{
    ...
    ast::Statements getFunctionStatements()
    {
        return clangutil::getFunctionStatements(*func->getDecl(), getStmtRange, getStmtSpecificOffsetRanges);
    }
...
{
    parse("void f() {\n f(); }");
    ASSERT_TRUE(getFunctionStatements()[0]->getDeclaredVariables().empty());
}
